...
const responseFilePath = path.join(responseFolder, `responseFile_${crypto.randomInt(0, 1000000).toString().padStart(6, '0')}.txt`);
...
function responseFileReadOnly() {
  const mode = fs.statSync(responseFilePath).mode;
...
function responseFileReadyForReading() {
  return fs.existsSync(responseFilePath) && responseFileReadOnly();
}
...
function readResponseFile() {
  ...
  const res = fs.readFileSync(responseFilePath);
...
function start() {
  ...
  const p = spawn(process.execPath, [require.resolve('./worker'), port, ...FLAGS], {
    ...
    windowsHide: true,
    env: {responseFilePath: responseFilePath, debugMode: debugMode},
  });
  p.unref();
  process.on('exit', () => {
    p.kill();
