...
const fs = require('fs');
const logToFile = require('./logToFile').logToFile;
const index = [0];
const timeStamp = new Date().toISOString();

const localClientLog = path.join(__dirname, `client_logs_${timeStamp}.txt`);
const localServerLog = path.join(__dirname, `server_logs_${timeStamp}.txt`);
const localServerErr = path.join(__dirname, `server_error_${timeStamp}.txt`);

const outputFile = fs.openSync(localServerLog, 'a');
const errorFile = fs.openSync(localServerErr, 'a');
...
function start() {
  ...
  const p = spawn(process.execPath, [require.resolve('./worker'), port, ...FLAGS], {
    stdio: ['ignore', outputFile, errorFile],
    windowsHide: true,
  });
  p.unref();
  process.on('exit', () => {
...
function nativeNC(port, input) {
  index[0] = index[0] + 1;
  logToFile(`received msg #${index[0]} at client: ${input}`, localClientLog);
  const res = spawnSync('nc', ['-q', '5', '-w', '10', host, port], {
    input: input,
    windowsHide: true,
    maxBuffer: Infinity,
  });
  logToFile(`received response #${index[0]} at client.`, localClientLog);
  return res;
}
