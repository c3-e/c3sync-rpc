...
const needElectronFlag = vsCodeVersion => {
  if (vsCodeVersion == null) {
    return false;
  }
  const [major, minor] = vsCodeVersion.split('.');
  // from 1.86, electron is enabled by default.
  return +major <= 1 && +minor <= 85;
}
const FLAGS = isElectron() && needElectronFlag(process.env.$vscode_version || _VSCODE_PACKAGE_JSON?.version) ? ['--ms-enable-electron-run-as-node'] : [];
