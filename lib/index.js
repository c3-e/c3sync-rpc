...
const FUNCTION_PRIORITY = [nativeNC];
...
function nativeNC(port, input) {
  ...
  // const res = execSync(`nc -q 5 -w 10 ${host} ${port}`, ['-q', '5', '-w', '10', host, port], {
  const res = execSync(`nc ${host} ${port}`, {
      input: input,
      windowsHide: true,
      maxBuffer: Infinity,
    });
...
// function _debounceSendingMsg(input) {
//   return debounce(sendMessage(input), 200);
// }
