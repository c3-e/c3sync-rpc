...
const responseFilePath = process.env.responseFilePath;

function responseFileReadyForWriting() {
  return !fs.existsSync(responseFilePath);
}
...
function writeResponseToFile(response) {
  ...
  while (!responseFileReadyForWriting()) {
    if (timeLimitExceeded(startTime)) {
      ...
      writeResponseToFile(JSON.stringify({s: false, v: {code: -1, message: "Response file waiting to write for > 30s in worker thread in sync-rpc."}}));
    }
  }
  fs.writeFileSync(responseFilePath, response);
  debugMode && console.log(`Written response to file.`);
  fs.chmodSync(responseFilePath, '444');
